...
io.on('connection', function(socket) {
  ...
  socket.on('joinGame', msg =>{
    if(gameHandler.roomCodeExists(msg.roomCode.toUpperCase())){
      gameHandler.nameSpaces[msg.roomCode.toUpperCase()].game.addUser({userName : msg.userName , socket : socket.id });
    } else {
  ...
  socket.on('startNewGame', msg => {
    ...
    gameHandler.nameSpaces[ns].game.isPicking = { userName : msg.userName , socket : socket.id }
    gameHandler.nameSpaces[ns].game.addUser({ userName : msg.userName , socket : socket.id })
    socket.emit('newRoomCode' , { roomCode : ns } )
  })

  socket.on('addUserName', msg => {
    if (gameHandler.users.length === 0) {
      gameHandler.gameisPicking = { userName: msg, socket: socket.id };
      socket.emit('youArePicking');
    }
    gameHandler.gameaddUser({ userName: msg, socket: socket.id });
